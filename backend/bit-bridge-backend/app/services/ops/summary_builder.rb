    ...
    def webhooks_section
      {
        ...
        providers: {
          ...
          monnify: non_persisted_webhook_metrics,
          bridgecard: non_persisted_webhook_metrics
        }
      }
    end

    def webhook_event_metrics(source:)
      scoped = WebhookEvent.where(source: source)
      {
        ...
        failed_last_24h: scoped.where(created_at: window_start..now).where.not(processing_error: [nil, '']).count,
